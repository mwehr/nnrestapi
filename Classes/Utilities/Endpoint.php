...
class Endpoint extends \Nng\Nnhelpers\Singleton {
	...
	public function find( $reqType = '', $controllerName = '', $methodName = '', $extSlug = '' ) 
	{
		...
		$controllerName = $this->kebabToCamelCase( $controllerName );
		$methodName = $this->kebabToCamelCase( $methodName );
	...
	/**
	 * Convert `test-this` to `testThis`
	 * (kebab to camelCase)
	 * ```
	 * $camelCase = \nn\rest::Endpoint()->kebabToCamelCase('some-string');
	 * ```
	 * @param string $str
	 * @return string
	 */
	public function kebabToCamelCase( $str = '' ) 
	{
		if (strpos($str, '-') === false) {
			return $str;
		}
