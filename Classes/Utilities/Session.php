...
class Session extends \Nng\Nnhelpers\Singleton 
{
	 ...
	 * We are __not__ using the standard Model/Repository methods here to increase performance.
     * ```
	 * \nn\rest::Session()->update( $identifier, ['sid'=>'...'], false, 123 );
	 * \nn\rest::Session()->update( $identifier, ['sid'=>'...'], ['my'=>'vars'], 123 );
	 * \nn\rest::Session()->update( $identifier, ['sid'=>'...'], ['my'=>'vars'], 123 );
	 * ```
	 * @return array
	 */
	public function update( $token = null, $data = [], $vars = false, $feUserUid = 0 ) 
	{
	 ...
	 * Alias to `update()`
	 * ```
	 * \nn\rest::Session()->create( $identifier, ['sid'=>'...'], [], 123 );
	 * \nn\rest::Session()->create( $identifier, ['sid'=>'...'], ['my'=>'vars'], 123 );
	 * ```
	 * @return array
	 */
	public function create( $token = null, $data = [], $vars = false, $feUserUid = 0 ) 
	{
		return $this->update( $token, $data, $vars, $feUserUid );
	}
	...
	/**
	 * Destroys a session for a certain `fe_user`
	 * ```
	 * // automatically use the current `fe_user.uid`
	 * \nn\rest::Session()->logoutFeuser();
	 * 
	 * // logout a other user by the uid
	 * \nn\rest::Session()->logoutFeuser( 123 );
	 * ```
	 * @param int $feUserUid
	 * @return boolean
	 */
	public function logoutFeuser( $feUserUid = 0 ) 
	{
		$feUserUid = intval($feUserUid) ?: \nn\t3::FrontendUser()->getCurrentUserUid();
		if (!$feUserUid) {
			return false;
		}
		\nn\t3::Db()->delete( self::TABLENAME, ['feuser'=>$feUserUid] );
		\nn\t3::Db()->delete( 'fe_sessions', ['ses_userid'=>$feUserUid] );
		return true;
	}
