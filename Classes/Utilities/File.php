...
{	
	...
	public function processFileUploadsInRequest( $request ) 
	{
		...
		if (!isset($this->uploadConfig['allowedFileUploadSuffix'])) {
			$this->uploadConfig['allowedFileUploadSuffix'] = $settings['allowedFileUploadSuffix'] ?? '';
		}
	...
	public function processFileUpload( $placeholder = '', $uploadedFiles = [], $configuration = [] ) 
	{
		...
		if (strpos($placeholder, $this->uploadPrefix) === 0) {
			...
			$allowedSuffixes = \nn\t3::Arrays( $this->uploadConfig['allowedFileUploadSuffix'] ?? '' )->trimExplode();
			
			if (\nn\t3::File()->isForbidden($srcFileName, $allowedSuffixes)) {
